...
/**
 * Test for cases when db network connectivity goes down and close is called on existing
 * connections. By default Hikari
 * blocks longer than getMaximumTimeout (it can hang for a lot of time depending on driver
 * timeout settings). Closing
...
public class TestConnectionCloseBlocking {
   ...
   public void testConnectionCloseBlocking() throws SQLException {
      ...
      try (HikariDataSource ds = new HikariDataSource(config)) {
         ...
         // Hikari only checks for validity for connections with lastAccess > 1000 ms so we sleep
         // for 1001 ms to force
         // Hikari to do a connection validation which will fail and will trigger the connection
         // to be closed
         ...
         Assert.assertTrue("Waited longer than timeout",
                           (ClockSource.INSTANCE.elapsedMillis(start) <
