...
abstract class PoolBase {
   ...
   private static final String[] RESET_STATES  =
      {"readOnly", "autoCommit", "isolation", "catalog", "netTimeout"};
   ...
   PoolBase(final HikariConfig config) {
      ...
      this.transactionIsolation =
         UtilityElf.getTransactionIsolation(config.getTransactionIsolation());
   ...
   boolean isConnectionAlive(final Connection connection) {
      try {
         if (isUseJdbc4Validation) {
            return connection
               .isValid((int) MILLISECONDS.toSeconds(Math.max(1000L, validationTimeout)));
         }
         ...
         try (Statement statement = connection.createStatement()) {
            if (isNetworkTimeoutSupported != TRUE) {
               setQueryTimeout(statement,
                               (int) MILLISECONDS.toSeconds(Math.max(1000L, validationTimeout)));
            }
         ...
         LOGGER.warn("{} - Failed to validate connection {} ({})", poolName, connection,
                     e.getMessage());
         return false;
      }
   }
      ...
      if (isQueryTimeoutSupported != FALSE) {
         try {
            ...
            if (isQueryTimeoutSupported == UNINITIALIZED) {
               ...
               LOGGER.warn("{} - Failed to set query timeout for statement. ({})", poolName,
                           e.getMessage());
            }
         }
      }
      ...
      if (isNetworkTimeoutSupported != FALSE) {
         try {
            ...
            if (isNetworkTimeoutSupported == UNINITIALIZED) {
               ...
               LOGGER.warn("{} - Failed to get/set network timeout for connection. ({})", poolName,
                           e.getMessage());
   ...
   private void setLoginTimeout(final DataSource dataSource, final long connectionTimeout) {
      if (connectionTimeout != Integer.MAX_VALUE) {
         try {
            dataSource
               .setLoginTimeout((int) MILLISECONDS.toSeconds(Math.max(1000L, connectionTimeout)));
         } catch (Throwable e) {
            LOGGER.warn("{} - Failed to set login timeout for data source. ({})", poolName,
                        e.getMessage());
         }
      }
   }
