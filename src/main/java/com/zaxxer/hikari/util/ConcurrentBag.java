...
public class ConcurrentBag<T extends IConcurrentBagEntry> implements AutoCloseable {
   ...
   public boolean remove(final T bagEntry) {
      if (!bagEntry.compareAndSet(STATE_IN_USE, STATE_REMOVED) &&
         ...
         LOGGER
            .warn("Attempt to remove an object from the bag that was not borrowed or reserved: {}",
                  bagEntry);
         return false;
      }
   ...
   public void unreserve(final T bagEntry) {
      if (bagEntry.compareAndSet(STATE_RESERVED, STATE_NOT_IN_USE)) {
         ...
         LOGGER
            .warn("Attempt to relinquish an object to the bag that was not reserved: {}", bagEntry);
      }
   }
