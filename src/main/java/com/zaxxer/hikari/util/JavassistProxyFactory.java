...
public final class JavassistProxyFactory {
   ...
   public static void main(String... args) {
      ...
      try {
         ...
         String methodBody =
            "{ try { return delegate.method($$); } catch (SQLException e) { throw checkException" +
            "(e); } }";
         ...
         methodBody =
            "{ try { return ((cast) delegate).method($$); } catch (SQLException e) { throw " +
            "checkException(e); } }";
   ...
   private static <T> void generateProxyClass(Class<T> primaryInterface, String superClassName,
      ...
      // Make a set of method signatures we inherit implementation for, so we don't generate
      // delegates for these
      ...
      for (Class<?> intf : interfaces) {
         ...
         for (CtMethod intfMethod : intfCt.getDeclaredMethods()) {
            ...
            // If the super-Proxy has concrete methods (non-abstract), transform the call into a
            // simple super.method() call
            CtMethod superMethod =
               superCt.getMethod(intfMethod.getName(), intfMethod.getSignature());
            if ((superMethod.getModifiers() & Modifier.ABSTRACT) != Modifier.ABSTRACT &&
                !isDefaultMethod(intf, intfCt, intfMethod)) {
            ...
            if (isThrowsSqlException(intfMethod)) {
               ...
               modifiedBody =
                  "{ return ((cast) delegate).method($$); }".replace("method", method.getName())
                                                            .replace("cast",
                                                                     primaryInterface.getName());
            }
   ...
   private static boolean isDefaultMethod(Class<?> intf, CtClass intfCt,
                                          CtMethod intfMethod) throws Exception {
      ...
      return intf
         .getDeclaredMethod(intfMethod.getName(), paramTypes.toArray(new Class[paramTypes.size()]))
