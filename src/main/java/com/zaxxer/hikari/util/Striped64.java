...
abstract class Striped64 extends Number
{
    ...
    */
   final void doubleAccumulate(double x, boolean wasUncontended)
   {
      int h;
      if ((h = getProbe()) == 0) {
         ThreadLocalRandom.current(); // force initialization
      ...
      for (;;) {
         ...
         if ((as = cells) != null && (n = as.length) > 0) {
            if ((a = as[(n - 1) & h]) == null) {
               ...
               wasUncontended = true; // Continue after rehash
            else if (a.cas(v = a.value, (Double.doubleToRawLongBits(Double.longBitsToDouble(v) + x))))
               break;
            else if (n >= NCPU || cells != as)
               collide = false; // At max size or stale
            else if (!collide)
         ...
         else if (casBase(v = base, (Double.doubleToRawLongBits(Double.longBitsToDouble(v) + x))))
            break; // Fall back on using base
      }
   }
